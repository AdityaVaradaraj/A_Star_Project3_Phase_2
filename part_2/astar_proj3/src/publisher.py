...
class Graph:
    def __init__(self, start, end, RPM1, RPM2, r, cl):
        self.visited = {}
        self.endI = end.i
        self.endJ = end.j
        self.RPM1 = RPM1
    ...
    def performAStar(self, start, end):
        ...
        if self.isAnObstacle(start.i, start.j) and self.isAnObstacle(end.i, end.j):
            rospy.loginfo("Starting and ending point are inside the obstacle! Check cls!")
            return

        if self.isAnObstacle(start.i, start.j):
            rospy.loginfo("Starting point is inside the obstacle! Check cls!")
            return

        if self.isAnObstacle(end.i, end.j):
            rospy.loginfo("Ending point is inside the obstacle! Check cls!")
            return

        if self.isOutsideArena(start.i, start.j):
            rospy.loginfo("Starting point is outside the arena! Check cls!")
            return

        if self.isOutsideArena(end.i, end.j):
            rospy.loginfo("Ending point is outside the arena! Check cls!")
            return

        rospy.loginfo("Started A-star")
        priorityQueue = []
        visited_list = {}
        ...
        while len(priorityQueue):
            ...
            if self.isInTargetArea(currentNode.i, currentNode.j):
                ...
                print("Found a path")
                print("Distance from start to goal is:", currentNode.costToCome)
                return True
...
def publisher(action):
    msg = Twist()
    my_pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)
